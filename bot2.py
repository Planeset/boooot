...
def start(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_photo(photo=photo_url, caption='''Привет, дорогой покупатель!👋
...
def actions_panel(update: Update, context: CallbackContext) -> None:
...
def handle_message(update: Update, context: CallbackContext) -> None:
    ...
    if text == "Заказать товар":
        update.message.reply_text("Для оформления заказа напишите @assistant_skinhub")
        ...
        update.message.reply_text('Каталог Skin.Hub Cosmetics🔤:\nМы предоставляем обширный список брендов и товаров, которые вы, уважаемые клиенты, можете у нас заказать. ✅\нНаш магазин предлагает вам ознакомиться с каталогом! Мы составили его для вашего удобства, для возможности посмотреть наличие товара или возможность его \nоформить под заказ. 🔔\нПриятных покупок!🤞', reply_markup=reply_markup)
...
def button(update: Update, context: CallbackContext) -> None:
    ...
    if query.data.startswith('brand_'):
        ...
        if brand != 'Другое...':
            ...
            product_list = "\n -".join(products)
            query.edit_message_text(text=f"Каталог товаров для бренда {brand}:\n{product_list}")
        else:
            query.edit_message_text(text="Пожалуйста, напишите @assistant_skinhub для получения подробной информации о товарах других брендов.")

def main() -> None:
    updater = Updater("7025335462:AAHVxTrH4hx7vwo0bmE3YQxzc0zMk1QJwDs", use_context=True)
